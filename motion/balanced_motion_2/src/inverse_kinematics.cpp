...
static rclcpp::Logger logger = rclcpp::get_logger("Inverse Kinematics");

static const float thigh = 0.10000;
static const float tibia = 0.10290;
static const float ankle = 0.04519;

nao_interfaces::msg::Joints calculate_joints(
  motion_msgs::msg::IKCommand & ik_command)
{
  ...
  if (legXL > thigh + tibia) {
    RCLCPP_ERROR(
      logger, "(Walk2014Generator) cannot solve Inverse Kinematics for that foot position!");
  }
